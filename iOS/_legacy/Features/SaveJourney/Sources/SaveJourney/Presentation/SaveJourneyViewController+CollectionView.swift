...
extension SaveJourneyViewController {
    ...
    func configureDataSource() -> SaveJourneyDataSource {
        ...
        let headerRegistration = HeaderRegistration(elementKind: SaveJourneyHeaderView.elementKind)                                                    { [weak self] header, _, indexPath in
        ...
        let dataSource = SaveJourneyDataSource(collectionView: self.collectionView)                                               { collectionView, indexPath, item in
            switch item {
            case .music(let song):
